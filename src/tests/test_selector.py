...
def test_net_fee_selector_q1_2024():
    ...
    proofs = hybrid.extract_metric_from_file(
        p, "net_fee_and_commission_income", period_label="Q1 2024"
    )
    # ensure we got candidates
    assert (
        isinstance(proofs, list) and len(proofs) >= 1
    ), "No proofs returned for test file"
    ...
    assert (
        "net fee and commission income" in top["raw_line"].lower()
        or "net fee and commission" in top["raw_line"].lower()
    )
    assert (
        top["confidence"] >= 0.45
    ), f"Top candidate confidence too low: {top['confidence']}"
