...
def roe(net_income: float, avg_equity: float) -> ProofObject:
    ...
    if avg_equity == 0:
        return ProofObject(
            ...
            inputs={"net_income": net_income, "avg_equity": avg_equity},
            output=None,
            note="division_by_zero",
        )
...
def loan_to_deposit(loans: float, deposits: float) -> ProofObject:
    if deposits == 0:
        return ProofObject(
            ...
            inputs={"loans": loans, "deposits": deposits},
            output=None,
            note="division_by_zero",
        )
    val = loans / deposits
...
def compute_roe_series(
    ...
    """
    results = {}
        pass  # TODO: Fix .items() call -     for period, metrics in extracted_metrics.items():  # type: ignore
        ni = (
            metrics.get("net_income")
            or metrics.get("net_profit")
            or metrics.get("netprofit")
        )
