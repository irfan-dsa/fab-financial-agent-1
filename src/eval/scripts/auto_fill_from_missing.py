...
lines = [l.strip() for l in MISSING_TSV.read_text(encoding='utf-8', errors='ignore').splitlines() if l.strip()]
if not lines:
...
def find_files_for_period(period_str):
    ...
    if not m:
        ...
        for f in RAW_DIR.glob('*'):
            if period_str.replace('_','-') in f.name or period_str.replace('_',' ') in f.name:
                candidates.append(f)
        return candidates
    q = m.group(1)
    year = m.group(2)
...
for ln in lines:
    ...
    for f in file_candidates:
        try:
            proofs = hybrid.extract_metric_from_file(str(f), metric.strip(), period_label=period.strip())
    ...
    if not proofs:
        print('  NO_PROOFS_RETURNED for', metric, 'period', period, 'candidates:', [p.name for p in file_candidates])
        skipped.append((file_candidates, period, metric))
        continue

    # select best via selector when available
    try:
        best = post_process_proofs_select_best(proofs, metric.replace('_',' '), metric, top_n=1, return_all=False)
