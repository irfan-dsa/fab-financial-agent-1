...
def generate_evaluation_report(report_json_path: str):
    ...
    md.append(
        f"- **Average Citation Score:** {metrics['avg_citation_score']*100:.1f}%\n"
    )
    ...
    if total_numeric > 0:
        md.append(
            f"- **Queries within tolerance:** {len(accurate_queries)}/{total_numeric}\n"
        )
        md.append(
            f"- **Accuracy rate:** {(len(accurate_queries)/total_numeric)*100:.1f}%\n"
        )

        if accurate_queries:
            avg_error = sum(
                r["accuracy"]["relative_error"] for r in accurate_queries
            ) / len(accurate_queries)
    ...
    retrieval_queries = [
        r for r in report["detailed_results"] if r.get("retrieval_metrics")
    ]

    if retrieval_queries:
        avg_precision = sum(
            r["retrieval_metrics"]["precision"] for r in retrieval_queries
        ) / len(retrieval_queries)
        avg_recall = sum(
            r["retrieval_metrics"]["recall"] for r in retrieval_queries
        ) / len(retrieval_queries)
        avg_f1 = sum(
            r["retrieval_metrics"]["f1_score"] for r in retrieval_queries
        ) / len(retrieval_queries)
    ...
    citation_queries = [
        r for r in report["detailed_results"] if r.get("citation_metrics")
    ]

    if citation_queries:
        has_source = sum(
            1 for r in citation_queries if r["citation_metrics"]["has_source"]
        ) / len(citation_queries)
        has_page = sum(
            1 for r in citation_queries if r["citation_metrics"]["has_page"]
        ) / len(citation_queries)
        mentions_source = sum(
            1 for r in citation_queries if r["citation_metrics"]["mentions_source"]
        ) / len(citation_queries)
        ...
        md.append(
            f"- **Overall citation score:** {metrics['avg_citation_score']*100:.1f}%\n"
        )
    md.append("\n")
    ...
    times = sorted(
        [r["response_time"] for r in report["detailed_results"] if r.get("success")]
    )
    if times:
    ...
    md.append(
        f"- **Projected cost (1000 queries):** ${metrics['avg_cost_per_query']*1000:.2f}\n"
    )
    md.append("\n")
    ...
    for i, result in enumerate(successful, 1):
        ...
        if result.get("accuracy"):
            md.append(
                f"**Accuracy:** {'✅ Within tolerance' if result['accuracy']['within_tolerance'] else '❌ Outside tolerance'}\n\n"
            )
    ...
    if report["failure_analysis"]:
        ...
        for failure in report["failure_analysis"]:
            ...
            md.append(
                "**Analysis:** This failure indicates [need for improvement in X area]\n\n"
            )
    else:
    ...
    md.append("### Strengths\n")
    md.append(
        f"1. High accuracy on simple factual queries ({metrics['accuracy_rate']*100:.1f}%)\n"
    )
    ...
    md.append(
        f"The FAB Financial Analysis Agent demonstrates strong performance with a "
    ...
    md.append(
        f"With an average cost of ${metrics['avg_cost_per_query']:.6f} per query and "
