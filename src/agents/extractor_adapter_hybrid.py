...
def extract_metric_from_file(
    ...
    if mapped_metric:
        try:
            ...
            if mapped_metric in extractor.FAB_LINE_ITEMS:
                ...
                if quarter and year:
                    logging.info(f'Direct extraction for {metric} from {quarter} {year}')
                    ...
                    for period_type in range(4):
                        ...
                        if value and 'error' not in metadata:
                            period_name = metadata.get('period', f'Period {period_type}')
    ...
    return _page_scan_fallback(str(path), metric, max_pages=None, require_keyword=require_keyword)
