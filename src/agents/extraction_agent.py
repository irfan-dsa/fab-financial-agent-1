...
def extract_metrics_from_chunk(chunk: Dict[str, Any]) -> List[ExtractedMetric]:
    ...
    if (
        chunk.get("chunk_type") == "table_row"
        and chunk.get("table_header")
        and chunk.get("table_row")
    ):
...
def fallback_extract_all_metrics(pdf_file: str, metrics: list):
    """
    Try extracting multiple metrics directly from the PDF when vector chunks fail.
    Returns a list of proof objects.
    """
    proofs = []
    for metric in metrics:
        result = _page_scan_fallback(
            pdf_file, metric, max_pages=None, require_keyword=True
        )
        if result:
