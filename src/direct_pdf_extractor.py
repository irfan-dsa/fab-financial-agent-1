...
FAB_LINE_ITEMS = {
    ...
    'Income from Islamic financing': ['income from islamic financing', 'income from islamic financing and investing products'],
    'Distribution on Islamic deposits': ['distribution on islamic deposits', 'distribution on islamic customers'],
...
def extract_values_from_line(line: str, search_terms: List[str]) -> Optional[List[str]]:
    ...
    for term in search_terms:
        if term.lower() in line.lower():
            ...
            if len(values) >= 4:  # Should have 4 values (9M curr, 9M prior, 3M curr, 3M prior)
                return values
    return None
...
def get_quarter_pdf_path(quarter: str, year: int, data_dir: str = "data/raw") -> Optional[str]:
