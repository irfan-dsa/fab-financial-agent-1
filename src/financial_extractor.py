...
def extract_financial_value(content: str, label: str, column_index: int = 0) -> str:
    """
        ...
        if label.lower() in line.lower():
            ...
            for j in range(i + 1, min(i + 5, len(lines))):
                ...
                if re.search(r'\d', potential_value_line):
                    ...
                    values = [v.strip() for v in potential_value_line.split() if v.strip()]
...
def query_financial_metric(client, encoder, metric_name: str, quarter_year: str, column_index: int = 0):
    """
...
if __name__ == "__main__":
    ...
    for metric, quarter, col_idx, description in test_cases:
        value, source = query_financial_metric(client, encoder, metric, quarter, col_idx)
