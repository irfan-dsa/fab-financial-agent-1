...
class EvaluationMetrics:
    ...
    def calculate_accuracy(
        self, predicted_value: float, ground_truth_value: float, tolerance: float = 0.01
    ) -> Dict:
        """
        ...
        if ground_truth_value == 0:
            ...
            relative_error = abs(
                (predicted_value - ground_truth_value) / ground_truth_value
            )
    ...
    def evaluate_retrieval(
        self, retrieved_docs: List[Dict], relevant_periods: List[str]
    ) -> Dict:
        """
        Calculate retrieval precision and recall
        """
        retrieved_periods = set(
            [
                doc.get("quarter_year")
                for doc in retrieved_docs
                if doc.get("quarter_year")
            ]
        )
...
class EvaluationRunner:
    ...
    def run_single_query(self, query_data: Dict) -> Dict:
        ...
        try:
            ...
            if query_data["category"] == "simple_factual":
                ...
                result.update(
                    self._evaluate_multihop(query_data, answer, retrieved_docs)
                )
            ...
            # Universal metrics
            result["citation_metrics"] = self.metrics.evaluate_citations(
                answer, metadata
            )
    ...
    def _evaluate_factual(self, query_data: Dict, answer: str, metadata: Dict) -> Dict:
        ...
        if numbers and gt.get("expected_value"):
            ...
            accuracy = self.metrics.calculate_accuracy(
                predicted_value, gt["expected_value"]
            )
    ...
    def _evaluate_calculation(
        self, query_data: Dict, answer: str, metadata: Dict
    ) -> Dict:
        ...
        if numbers and gt.get("expected_value"):
            ...
            accuracy = self.metrics.calculate_accuracy(
                predicted_value, gt["expected_value"], tolerance
            )
    ...
    def _evaluate_multihop(
        self, query_data: Dict, answer: str, retrieved_docs: List
    ) -> Dict:
        """Evaluate multi-hop reasoning"""
        gt = query_data["ground_truth"]

        # Check if multiple periods were retrieved
        periods_retrieved = set(
            [
                doc.get("quarter_year")
                for doc in retrieved_docs
                if doc.get("quarter_year")
        ...
        retrieval_metrics = self.metrics.evaluate_retrieval(
            retrieved_docs, expected_periods
        )

        # Check if all required metrics mentioned
        metrics_needed = gt.get("metrics_needed", [])
        metrics_mentioned = sum(
            1 for metric in metrics_needed if metric.replace("_", " ") in answer.lower()
        )
        metric_coverage = (
            metrics_mentioned / len(metrics_needed) if metrics_needed else 0
        )
        ...
        return {
            ...
            "reasoning_depth": len(
                answer.split(". ")
            ),  # Rough proxy for reasoning depth
        }
    ...
    def _evaluate_temporal(self, query_data: Dict, answer: str, metadata: Dict) -> Dict:
        ...
        periods_mentioned = sum(
            1 for period in periods if period.replace("_", " ") in answer
        )
        ...
        return {
            ...
            "trend_identified": any(
                word in answer.lower()
                for word in ["increase", "decrease", "trend", "growth", "decline"]
            ),
        }
    ...
    def _calculate_aggregate_metrics(self) -> Dict:
        ...
        accuracy_rate = (
            ...
            if any(
                r.get("category") in ["simple_factual", "calculation"]
                for r in successful_results
            )
            else 0
        )
        ...
        # Citation quality
        citation_scores = [
            r["citation_metrics"]["citation_score"]
            for r in successful_results
            if r.get("citation_metrics")
        ]
        avg_citation_score = (
            sum(citation_scores) / len(citation_scores) if citation_scores else 0
        )
    ...
    def _breakdown_by_category(self) -> Dict:
        ...
        for category in [
            ...
            category_results = [
                r for r in self.results if r.get("category") == category
            ]
            ...
            if category_results:
                ...
                breakdown[category] = {
                    ...
                    "avg_response_time": sum(r["response_time"] for r in successful)
                    / len(successful)
                    if successful
                    else 0,
                }
    ...
    def _print_summary(self, report: Dict):
        ...
        for cat, data in report["category_breakdown"].items():
            print(
                f"  {cat}: {data['successful']}/{data['total']} ({data['success_rate']*100:.1f}%)"
            )
