...
from agents.extractor_adapter_hybrid import (METRIC_MAPPING,
                                             extract_metric_from_file)
...
class FABFinancialAgent:
    ...
    def _fallback_retrieve(self, state):
        ...
        if state["plan"]["quarters_mentioned"]:
            ...
            qys = [
                f"Q{re.match(r'Q(\d)\s*(\d{4})', q, re.I).group(1)}_{re.match(r'Q(\d)\s*(\d{4})', q, re.I).group(2)}"
                for q in state["plan"]["quarters_mentioned"]
            ]
            query_filter = Filter(
                must=[FieldCondition(key="quarter_year", match=MatchAny(any=qys))]
            )
            print(f"  Filtering: {qys}")

        results = self.vector_client.query_points(
            "fab_financials", query_vector, query_filter=query_filter, limit=20
        )
        state["retrieved_chunks"] = [
            {
        ...
        for chunk in state["retrieved_chunks"]:
            ...
            try:
                ...
                if proofs:
                    ...
                    print(
                        f"  ✓ {chunk['quarter']} {chunk['year']}: {proof['value_millions']:,.2f}M"
                    )
    ...
    def calculate_results(self, state):
        ...
        if state["plan"]["needs_calculation"] and len(state["extracted_metrics"]) >= 2:
            vals = sorted(
                state["extracted_metrics"].values(),
