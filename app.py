...
# Header
st.markdown(
    '<div class="main-header">🏦 FAB Financial Analyst AI</div>', unsafe_allow_html=True
)
st.markdown("---")
...
if analyze_btn and query:
    ...
    with st.spinner("🤖 Agent is working..."):
        try:
            ...
            with col_conf2:
                st.markdown(
                    f'<div class="{conf_class}">{conf_emoji} Confidence: {confidence:.1%}</div>',
                    unsafe_allow_html=True,
                )
...
if st.session_state.history:
    ...
    for i, item in enumerate(reversed(st.session_state.history[-5:]), 1):
        with st.expander(
            f"{i}. {item['query'][:60]}... ({item['timestamp'].strftime('%H:%M:%S')})"
        ):
